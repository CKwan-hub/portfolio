...
export function TimelineSection() {
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      const validItemEntries = Object.entries(itemRefs.current).filter(([_, ref]) => ref !== null)
  ...
  return (
    <section id="timeline" className="py-24 bg-muted/50 relative">
      ...
      <div className="container mx-auto px-4 relative">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[300px_1fr] gap-8" ref={timelineRef}>
          {/* Sticky Sidebar */}
              ...
              <div className="space-y-6">
                <div>
                  <h3 className="text-lg font-semibold mb-1 text-primary">
                    {timelineItems.find((item) => item.id === activeItemId)?.company}
                  </h3>
                  <p className="text-sm text-muted-foreground">
                    {timelineItems.find((item) => item.id === activeItemId)?.period}
                  </p>
                </div>
                ...
                >
                  ...
                  >
                    <span
                      className={cn(
                        ...
                        activeItemId === item.id ? "bg-primary" : "bg-muted-foreground/40",
                      )}
                    />
                  </div>
                  ...
                  <div
                    className={cn(
                      ...
                      activeItemId === item.id ? "border-primary/20 shadow-lg shadow-primary/5" : "border-border/50",
                    )}
                  >
                    {/* Mobile-only header info */}
                    <div className="lg:hidden mb-4">
                      <h3 className="text-lg font-semibold mb-1 text-primary">{item.company}</h3>
                    ...
                    <h3 className="text-xl font-semibold mb-3 text-foreground">{item.role}</h3>
                    ...
                    <div className="space-y-4">
                      {item.projects.map((project, idx) => (
                        <div key={idx} className="border-t border-border/50 pt-4">
                          <h4 className="font-medium mb-2 text-accent">{project.name}</h4>
                          <p className="text-sm text-muted-foreground mb-3">{project.description}</p>

                          {/* Placeholder for project images */}
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-4">
                            <div className="aspect-video bg-muted rounded-md flex items-center justify-center text-muted-foreground text-sm border border-border/30 hover:border-primary/30 transition-colors">
                              Project Screenshot
                            </div>
                            <div className="aspect-video bg-muted rounded-md flex items-center justify-center text-muted-foreground text-sm border border-border/30 hover:border-primary/30 transition-colors">
                              UI Example
